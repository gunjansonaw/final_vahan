...
from api_info import APIInfo, show_connection_status
...
def main():
    ...
    # Main content tabs
    tab1, tab2, tab3 = st.tabs(["📊 Analytics Dashboard", "🔌 API Information", "📋 Data Sources"])
    
    with tab2:
        APIInfo.display_api_info()
    
    with tab3:
        st.subheader("📋 Available Data Sources & Endpoints")
        
        st.markdown("""
        ### Official Vahan Dashboard Endpoints
        
        **Government Portal:** https://vahan.parivahan.gov.in/
        
        **Public Analytics:** https://analytics.parivahan.gov.in/analytics/publicdashboard/vahan
        
        **Vehicle Services:** https://vahan.parivahan.gov.in/vahaneservice/
        """)
        
        st.subheader("🏢 Authorized Third-Party APIs")
        
        providers = APIInfo.get_api_providers()
        
        for provider_id, info in providers.items():
            with st.expander(f"{info['name']} - API Endpoints"):
                st.write(f"**Description:** {info['description']}")
                st.write(f"**Documentation:** {info['documentation']}")
                st.write("**Available Endpoints:**")
                
                for endpoint_name, endpoint_url in info["endpoints"].items():
                    st.code(f"{endpoint_name}: {endpoint_url}")
        ...
        **Vehicle Categories:**
        ...
        - **3W:** Three-wheelers (auto-rickshaws, commercial vehicles)
    ...
    with tab1:
        st.subheader("📊 Vehicle Registration Analytics")
